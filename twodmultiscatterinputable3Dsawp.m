...
totalfield(PosStepsEnd:PosStepsStart,round(SampleHeight/2)+round(ysteps/2),round(zsteps/2)) = 2;
...
assignin("base","z1",z1)
assignin("base","totalfield",totalfield)
surf(x(:,:,round(ysteps/2)),y(:,:,round(ysteps/2)),real(totalfield(:,:,round(ysteps/2))))
view([0 90]);
shading interp;
figure(2)
colorbar
...
%Need to make this 3D (might not being in the right place yet
    function Slice = FourierSliceTaker(Field,FirstPoint,LastPoint,HeightAbove,xsteps,ysteps,zsteps)
        Slice = Field(FirstPoint:LastPoint,round(HeightAbove/2)+round(ysteps/2),round(HeightAbove/2)+round(ysteps/2));
    end
